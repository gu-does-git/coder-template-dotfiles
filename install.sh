...
log_step "CLI tools"
sudo apt install -y jq bat fzf eza ripgrep fd-find
...
log_step "timetrace"
curl -fsSL https://github.com/dominikbraun/timetrace/releases/download/v0.14.3/timetrace-linux-amd64.tar.gz | sudo tar -xz -C /usr/local/bin
...
bun install -g @servicenow/sdk @oh-my-pi/pi-coding-agent playwright vercel skillkit
